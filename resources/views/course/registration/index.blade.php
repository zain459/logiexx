...
@section('content')
    ...
    <div class="mb-3 d-flex justify-content-between align-items-center">
        <h3>Registration</h3>
        <a href="{{ route('course-class.registration.create', $class->id()) }}" class="btn btn-sm btn-primary">Add Registration</a>
    </div>
    <div class="card mb-3">
...
{{--                <th>Status</th>--}}
            ...
            @forelse($registrations as $registration)
                <tr>
                    <td>{{ $registration->form_no ?? '--' }}</td>
                    <td>{{ $registration->fullName() ?? '--' }}</td>
                    <td>{{ $registration->fatherHusbandName() ?? '--' }}</td>
                    <td>{{ $registration->cnic() ?? '--' }}</td>
                    <td>{{ $registration->passportNo() ?? '--' }}</td>
...
{{--                    @endif--}}
                    ...
                    <td class="table-action">
                        <a href="{{ route('course-class.registration.view', $registration->id()) }}">
